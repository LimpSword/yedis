...
static void buf_append(std::vector<uint8_t> &buf, const uint8_t *data, const size_t len) {
    buf.insert(buf.end(), data, data + len);
}

static void buf_consume(std::vector<uint8_t> &buf, const long n) {
    if (n > buf.size()) {
        buf.clear();
        return;
    }
...
static bool try_one_request(Conn *conn) {
    ...
    const auto *request = &conn->incoming[4];
    ...
    if (res_len > 0) {
        buf_append(conn->outgoing, reinterpret_cast<const uint8_t *>(&res_len), 4);
        buf_append(conn->outgoing, reinterpret_cast<const uint8_t *>(response.data()), res_len);
    }

    printf("client says: len:%d data:%s\n", len, reinterpret_cast<const char *>(request));
...
static void handle_write(Conn *conn) {
    ...
    const ssize_t rv = write(conn->fd, &conn->outgoing[0], conn->outgoing.size());
